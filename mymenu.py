...
class App(customtkinter.CTk):
    def __init__(self, userId):
        ...
        self.tabview._segmented_button.configure(font = customtkinter.CTkFont(family = "Verdana", size = 25, weight = "bold"),
                                                 height = 100,
                                                 width = 100,
                                                 fg_color = "#272727")
        ...
        self.label_username = customtkinter.CTkLabel(self.tabview.tab("  PROFILE  "), 
        ...
        self.label_wins = customtkinter.CTkLabel(self.tabview.tab("  PROFILE  "), 
                                                     font = customtkinter.CTkFont(family = "Verdana", size = 25), 
                                                     justify = "left")
        self.label_wins.grid(row = 3, column = 0, padx=30, pady=(20, 0), sticky = "w")

        self.label_registration_date = customtkinter.CTkLabel(self.tabview.tab("  PROFILE  "), 
                                                     font = customtkinter.CTkFont(family = "Verdana", size = 25), 
                                                     justify = "left")
        self.label_registration_date.grid(row = 4, column = 0, padx=30, pady=(5, 0), sticky = "w")

        self.label_header_tabel = customtkinter.CTkLabel(self.tabview.tab("  PROFILE  "), 
                                                     font = customtkinter.CTkFont(family = "Verdana", size = 20), 
                                                     text = "{:<34} {:<22} {:<6}".format("Date", "Enemy", "Result"), 
                                                     justify = "left")
        self.label_header_tabel.grid(row = 5, column = 0, padx=30, pady=(20, 10), sticky = "w")

        self.scrollable_frame = customtkinter.CTkScrollableFrame(master=self.tabview.tab("  PROFILE  "),
                                                                 width=WINDOW_WIDTH-130, 
                                                                 height=WINDOW_HEIGHT - 340, 
                                                                 border_color = "white",
                                                                 border_width = 3)
        self.scrollable_frame.grid(row = 6, column = 0, padx=30, pady=0, sticky = "w")
        self.scrollable_frame.grid_columnconfigure(0, weight=1)  # Для других элементов, которые могут растягиваться
        self.scrollable_frame.grid_columnconfigure(1, weight=0)
            
        self.label_leaders = customtkinter.CTkLabel(self.tabview.tab("  LEADERBOARD  "), 
                                                     font = customtkinter.CTkFont(family = "Verdana", size = 25), 
                                                     text = "{:<44} {:<4}".format("Username", "Wins"), 
                                                     justify = "left")
        self.label_leaders.grid(row = 0, column = 0, padx=30, pady=(30, 10), sticky = "w")

        self.scrollable_frame_leaders = customtkinter.CTkScrollableFrame(master=self.tabview.tab("  LEADERBOARD  "),
                                                                 width=WINDOW_WIDTH-130, 
                                                                 height=WINDOW_HEIGHT - 203, 
                                                                 border_color = "white",
                                                                 border_width = 3)
        self.scrollable_frame_leaders.grid(row = 1, column = 0, padx=30, pady=0)
        self.scrollable_frame_leaders.grid_columnconfigure(0, weight=1)  # Для других элементов, которые могут растягиваться
        self.scrollable_frame_leaders.grid_columnconfigure(1, weight=0)

        self.frame = customtkinter.CTkFrame(self.tabview.tab("  LOBBIES  "),
                                           width=WINDOW_WIDTH - 50, 
                                           height = 50,)
        self.frame.grid(row = 0, column = 0, padx=0, pady=0)
        self.frame.grid_columnconfigure(0, weight=1)  # Для других элементов, которые могут растягиваться
        self.frame.grid_columnconfigure(1, weight=0)

        self.button_create_lobby = customtkinter.CTkButton(self.frame,
                                                           width = 250,
                                                           height = 50,
                                                           text = "Create lobby",
        ...
        self.button_create_lobby.grid(row = 0, column = 1, padx = (30, 0), pady = (10, 20), sticky = "w")

        pil_image = Image.open("./img/refresh_icon.png")

        # Создание CTkImage из объекта PIL.Image
        self.image = customtkinter.CTkImage(pil_image)


        self.button_refresh_lobby = customtkinter.CTkButton(self.frame,
                                                           width = 50,
                                                           height = 50,
                                                           text = "",
                                                           image=self.image,
        ...
        self.button_refresh_lobby.grid(row = 0, column = 0, padx = 0, pady = (10, 20), sticky = "w")

        self.roundDurationVar = customtkinter.StringVar(value="30s")

        self.roundDurationMenu = customtkinter.CTkOptionMenu(self.frame,
                                                            values=["30s", "60s", "90s", "120s"],
        ...
        self.scrollable_frame_lobbies = customtkinter.CTkScrollableFrame(master=self.tabview.tab("  LOBBIES  "),
                                                                 width=WINDOW_WIDTH - 130, 
                                                                 height=WINDOW_HEIGHT - 212, 
                                                                 border_color = "white",
                                                                 border_width = 3)
        self.scrollable_frame_lobbies.grid(row = 2, column = 0, padx=30, pady=(0, 0))
        self.scrollable_frame_lobbies.grid_columnconfigure(0, weight=1)  # Для других элементов, которые могут растягиваться
    ...
    def showUserProfile(self, userId):
        ...
        self.label_registration_date.configure(text=f"Registration date: {self.user.getRegistrationDate().replace('T', ' ')}")
        ...
        for game in self.user.getGamesHistory():
            ...
            customtkinter.CTkLabel(self.scrollable_frame, 
                                   text = game_date,
                                   font = customtkinter.CTkFont(family = "Verdana", size = 16),
                                   justify = "left").grid(row=rowN, column=0, pady=0, padx=(5,0), sticky = "w")
            
            customtkinter.CTkLabel(self.scrollable_frame, 
                                   text = enemy_username,
                                   font = customtkinter.CTkFont(family = "Verdana", size = 16),
                                   justify = "left").grid(row=rowN, column=1, pady=0, padx=0, sticky = "w")
            ...
            if (win):
                customtkinter.CTkLabel(self.scrollable_frame, 
                                    ...
                                    font = customtkinter.CTkFont(family = "Verdana", size = 16),
                                    justify = "left", 
                                    width = 100).grid(row=rowN, column=2, pady=0, padx=0, sticky = "w")
            else:
                customtkinter.CTkLabel(self.scrollable_frame, 
                                    text="Lose",
                                    text_color = "#D44213",
    ...
    def showLeaders(self):
        ...
        for leader in self.leaders:
            customtkinter.CTkLabel(self.scrollable_frame_leaders,
            ...
            customtkinter.CTkLabel(self.scrollable_frame_leaders, 
                                   ...
                                   text_color = "#91AD54",
                                   font = customtkinter.CTkFont(family = "Verdana", size = 16),
                                   justify = "left",).grid(row=rowN, column=1, pady=0, padx=(0, 50), sticky = "e")
            rowN += 1
    ...
    def showLobbies(self):
        ...
        isMyIdInLobbies = len(list(filter(lambda lb: lb['hostid'] == self.userId, self.lobbies))) > 0 
        ...
        for lobby in self.lobbies:
            ...
            customtkinter.CTkLabel(self.scrollable_frame_lobbies, 
                                                            font = customtkinter.CTkFont(family = "Verdana", size = 25), 
                                                            text = f"{lobby['roundduration']}s | {lobby['username']}", 
                                                            justify = "left").grid(row = rowN, column = 0, padx = (10, 0), sticky = "w")
            customtkinter.CTkButton(self.scrollable_frame_lobbies,
                                                            font = customtkinter.CTkFont(family = "Verdana", size = 25), 
                                                            command=self.leaveLobby if button_text == "Leave" else lambda ip=lobby['hostip']: self.joinLobby(ip),
                                                            text = button_text,
                                                            state = myState).grid(row = rowN, column = 1, sticky = "e", padx = (0, 10), pady = (10, 0))
            rowN += 1
    ...
    def createLobby(self):
        self.db_manager.createLobby(self.userId, f"{self.get_local_ip()}:{self.connManager._port}", self.getRoundDurationVar())
        self.showLobbies()
    ...
    async def runAsync(self):
        await asyncio.gather(self.mainloop(), self.connManager.run())
    ...
    #             score, _  = await self.game(response["duration"])
    #             await websocket.send(json.dumps({"userId": self.userId,
    #                                         "message": "end game",
    #                                         "duration" : 10,
    #                                         "score": score}))
    ...
    #                 score, duration = await self.game()
    #                 await websocket.send(json.dumps({"userId": self.userId,
    #                                         "message": "fatality",
    #                                         "duration" : duration,
    #                                         "score": score}))
    ...
    #                     self.gm_result.append((True, "You win"))
    #                     self.db_manager.addGame(self.app.userId, response["userId"])
    #                 else: 
    #                     GameResult((False, "You lose"))
    #                     self.gm_result.append((False, "You lose"))
    #                     self.db_manager.addGame(response["userId"], self.app.userId)
