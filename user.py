...
class User():
    def __init__(self, userId,  port = 3000):
        ...
        self._id, self._username, self._registrationDate = self._database.getUserbyId(userId).values()
        self._gamesHistory = self._database.getUserGamesHistory(userId)
        self._winsCount = len(list(filter(lambda game: game["userwin"], self._gamesHistory)))
        self._connectionManger = ConnectionManager(port)
        #print(self._id, self._username, self._rigistrationDate, self._winsCount)
